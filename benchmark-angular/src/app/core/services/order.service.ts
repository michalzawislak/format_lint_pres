...
export class OrderService {
  private cache = new Map<number, OrderData>();
  private dataSubject = new BehaviorSubject<OrderData[]>([]);
  ...
  getById(id: number): Observable<OrderData | undefined> {
  ...
  create(data: Partial<OrderData>): Observable<OrderData> {
    const newItem: OrderData = {
      id: Date.now(),
      name: data.name || '',
  ...
  update(id: number, data: Partial<OrderData>): Observable<OrderData> {
  ...
  search(query: string): Observable<OrderData[]> {
  ...
  private generateMockData(): OrderData[] {
    return Array.from({ length: 20 }, (_, i) => ({
      id: i + 1,
      name: 'Order Item ' + (i + 1),
      description: 'This is a detailed description for order item number ' + (i + 1),
